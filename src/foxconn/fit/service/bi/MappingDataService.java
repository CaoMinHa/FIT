...
public class MappingDataService extends BaseService<Parameter>{
	...
	public List<String> index(HttpServletRequest request){
		...
		List<String> supplierList = this.listBySql("select t.lov_code||','||t.tab_name||'|'||t.lov_desc from CUX_MD_LOV_VALUES t where t.lov_type='RT_ACCOUNT' and t.enabled_flag='Y' and t.language='"+language+"' ORDER BY to_number(COL_SEQ)");
		return supplierList;
	}
	/**
	 * 獲取選中表查詢條件
	 */
	public List<List<String>> queryMasterData(HttpServletRequest request,String masterData){
		Locale locale = (Locale) WebUtils.getSessionAttribute(request,SessionLocaleResolver.LOCALE_SESSION_ATTRIBUTE_NAME);
		Assert.hasText(masterData, instrumentClassService.getLanguage(locale,"營收映射表不能为空","Master data can not be null"));
	...
	public void list(Model model,String language, PageRequest pageRequest, String masterData, String queryCondition) throws UnsupportedEncodingException {
		...
		Page<Object[]> page = this.findPageBySql(pageRequest, sql+" order by LAST_UPDATED_DATE desc,ID");
		int index=1;
	...
	public String upload(AjaxResult result, String masterData, HttpServletRequest request, Locale locale) throws Exception {
		...
		if (mutipartFiles != null && mutipartFiles.size() > 0) {
			...
			Assert.isTrue(tableName.equals(tableNAME), instrumentClassService.getLanguage(locale,"營收映射表選擇錯誤","The selected master data is error"));
