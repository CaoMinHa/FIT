        ...
        try {
            ...
            if (null != list && list.size() > 0) {
                ...
                if (dateString.equals(predate) || dateString.equals(poEmailLog.getEndDate()) || dateString.equals(predateTwo) || date.getTime() > date1.getTime()) {
                    String username = "";
                    String integer = poEmailLog.getEmailYear();
                    //查找未提交的SBU
...
//    @Scheduled(cron = "0 0 8 * * MON-SAT")
    public void checkProductDimension() {
        try {
            String sql = "SELECT distinct PRODUCT_SERIES_CODE,PRODUCT_SERIES_DESC FROM epmods.cux_inv_sbu_item_info_mv where PRODUCT_SERIES_DESC not in(select DIMENSION from FIT_DIMENSION where type='Product') and PRODUCT_SERIES_CODE not in('EWAA','3CAB','EVAE','5CD')";
            List<Map> product = poTableService.listMapBySql(sql);
            if (null != product && product.size() > 0) {
                String content = "Dear 系統管理員：</br>&nbsp;&nbsp;以下維度值還未維護，請儘快行動。<br></br><span style=\"white-space: pre;\">";
                for (Map m:product) {
                    content+="&nbsp;&nbsp;"+m.get("PRODUCT_SERIES_CODE").toString()+"："+m.get("PRODUCT_SERIES_DESC").toString()+"<br></br>";
                }
                content+="</span><br></br>Best Regards!";
//                EmailUtil.emailsMany("it-ks-mfg@fit-foxconn.com","預算系統 Product Series 維護",content);
                EmailUtil.emailsMany("arshi@deloitte.com.cn", "預算系統 Product Series 維護", content);
                EmailUtil.emailsMany("yanwanggz@deloitte.com.cn", "預算系統 Product Series 維護", content);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        String userList = "select BI_USER,BI_USERNAME,EMAIL,BI_GROUP,BI_PORTALPATH from BIDEV.Bi_user_list u where u.EMAIL is not null and instr(';'||BI_GROUP||';',';RT_Backlog;')> 0";
        ...
        if (null != userMap && userMap.size() > 0) {
            for (Map map : userMap) {
                content = "<div style=\"font-size:15px;\">Dear " + map.get("BI_USERNAME").toString() + "主管：<br></br>&nbsp;&nbsp;&nbsp;截止" + df.format(date);
                String text = "";
                ...
                if (null != userAuthorities && userAuthorities.size() > 0) {
                    content += " "+userAuthorities.toString().substring(1,userAuthorities.toString().length()-1)+" ";
                    str = "";
                    for (String s : userAuthorities) {
                        str += "'" + s + "',";
                    }
                    userNumber = "select t.c_type,t.YEAR_MONTH, ROUND(sum(t.REVENUE_ALL),0) REVENUE_ALL,ROUND(sum(t.TARGET_AMOUNT),0) TARGET_AMOUNT,ROUND(sum(t.REVENUE_MISC)+sum(t.REVENUE_MOVE),0) ACTUAL from epmebs.cux_actual_rev_blog_v t  WHERE SBU in(" + str.substring(0, str.length() - 1) + ")  group by t.YEAR_MONTH,t.c_type,t.YEAR_MONTH ORDER BY t.year_month DESC";
                } else {
                    content += " FIT ";
                    userNumber = "select t.c_type,t.YEAR_MONTH, ROUND(sum(t.REVENUE_ALL),0) REVENUE_ALL,ROUND(sum(t.TARGET_AMOUNT),0) TARGET_AMOUNT,ROUND(sum(t.REVENUE_MISC)+sum(t.REVENUE_MOVE),0) ACTUAL from epmebs.cux_actual_rev_blog_v t  WHERE SBU in('FIT')  group by t.YEAR_MONTH,t.c_type,t.YEAR_MONTH ORDER BY t.year_month DESC";
                }
                ...
                if (null != numberList && numberList.size() > 1) {
                    for (Map m : numberList) {
                        ...
                        if (Integer.valueOf(m.get("TARGET_AMOUNT").toString()) != 0) {
                            integer= (int) Math.round(Double.parseDouble(m.get("ACTUAL").toString())/Double.parseDouble(m.get("TARGET_AMOUNT").toString())*100);
                            integer1 = (int) Math.round(Double.parseDouble(m.get("REVENUE_ALL").toString()) / Double.parseDouble(m.get("TARGET_AMOUNT").toString()) * 100);
                        }
                        if (m.get("C_TYPE").toString().equals("month")) {
                            int a= (int)Math.round(((double)calendar.get(Calendar.DAY_OF_MONTH) / (double)calendar.getActualMaximum(Calendar.DAY_OF_MONTH) * 100));
                            text = "<span style=\"color: #000000;font-size:15px;\">幣別：MUSD</span><br></br>" +
                                    "<div style=\"color: #000000;font-size:17px;font-weight:bold;\">1 本月達成：</div>" +
                                    "<div><font style=\"color: #000000;font-size:15px;\">&nbsp;&nbsp;a.截止當日，達成率" + integer+ "%，時間進度" + a+ "%</font>" +
                                    "<font style=\"color: #0000FF;font-size:15px;\"> 【實際" + m.get("ACTUAL").toString() + ",目標" + m.get("TARGET_AMOUNT").toString() + "】</font></div>" +
                                    "<div><font style=\"color: #000000;font-size:15px;\">&nbsp;&nbsp;b.本月預估達成" + integer1 + "%</font>" +
                                    "<font style=\"color: #0000FF;font-size:15x;\"> 【實際+預估" + m.get("REVENUE_ALL").toString() + "，目標" + m.get("TARGET_AMOUNT").toString() + "】</font></div><br></br>";
                        } else if (m.get("C_TYPE").toString().equals("year")) {
                            text += "<div style=\"color: #000000;font-size:17px;font-weight:bold;\">2 YTD纍計達成：</div>" +
                                    "<div><font style=\"color: #000000;font-size:15px;\">&nbsp;&nbsp;a." + m.get("YEAR_MONTH").toString() + "纍計達成" + integer1 + "%</font>" +
                                    "<font style=\"color: #0000FF;font-size:15px;\"> 【實際+預估" + m.get("REVENUE_ALL").toString() + "，目標" + m.get("TARGET_AMOUNT").toString() + "】</font></div>";
                        } else if (m.get("C_TYPE").toString().equals("year2")) {
                            text += "<div><font style=\"color: #000000;font-size:15px;\">&nbsp;&nbsp;b.年度纍計達成" + integer1 + "%</font>" +
                                    "<font style=\"color: #0000FF;font-size:15px;\"> 【實際+預估" +m.get("REVENUE_ALL").toString() + "，目標" + m.get("TARGET_AMOUNT").toString()+ "】</font></div><br></br>    ";
                        }
                    }
                }
            ...
            if (null != userList && userList.size() > 0) {
                for (Map user : userList) {
                    sql = "select distinct COMMODITY_MAJOR from BIDEV.v_dm_d_commodity_major where COMMODITY_MAJOR not in(\n" +
                            ...
                            "where u.id=ur.user_id and r.id=ur.role_id and r.code in('SOURCER','specialSourcer') and u.type='BI' and u.username='" + user.get("USERNAME").toString() + "' and COMMODITY_MAJOR is not null)||',',','||COMMODITY_MAJOR||',')> 0";
                    ...
                    if (null != commodity.get(0) && commodity.get(0).length() > 0) {
                        ...
                        for (String c : commodityStr) {
                            sql = "select distinct u.email from fit_user u,FIT_PO_AUDIT_ROLE r ,FIT_PO_AUDIT_ROLE_USER ur where u.id=ur.user_id and r.id=ur.role_id and r.code in('CLASS','specialClass') and u.type='BI' and u.COMMODITY_MAJOR is not null\n" +
