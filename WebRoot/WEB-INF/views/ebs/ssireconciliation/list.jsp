...
function clickPage(page){
	...
	var TC=$("#QTC").val();
	$("#Content").load("${ctx}/hfm/ssireconciliation/list",{pageNo:$("#PageNo").val(),pageSize:$("#PageSize").val(),
		...
		<tbody>
			<c:forEach items="${page.result}" var="mapping">
				<tr>
					...
					<td style="border-right:1px solid #eee;text-align:center;">${mapping.ACCOUNT_CODE}</td>
					<td style="border-right:1px solid #eee;text-align:center;">${mapping.ACCOUNT_NAME}</td>
					...
					<td style="border-right:1px solid #eee;text-align:center;">${mapping.CREATED_BY}</td>
					<td style="border-right:1px solid #eee;text-align:center;">${mapping.CREATION_DATE}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
