...
import foxconn.fit.entity.ebs.SSIReconciliation;
import foxconn.fit.service.ebs.SSIReconciliationService;
...
@Controller
@RequestMapping("/hfm/ssireconciliation")
@SuppressWarnings("unchecked")
public class SSIReconciliationController extends BaseController{
    
	private static String EXCEL_NAME="科目餘額表";
	private static int COLUMN_NUM=19;
	
	@Autowired
	private SSIReconciliationService reconciliationService;
	...
	public String index(HttpServletRequest request, Model model) {
		...
		List<String> list = reconciliationService.listBySql("select distinct ENTITY_CODE from CUX_SSI_BAL where length(ENTITY_CODE)=6 order by ENTITY_CODE");
		if (list.size()>0) {
		...
		List<String> entityListssi = new ArrayList<String>();
		if (StringUtils.isNotEmpty(entity1)) {
			for (String code : entity1.split(",")) {
				if (code.length() == 8) {
				    entityListssi.add(code.substring(2));
				}
			}
		}
		model.addAttribute("entityList", entityList);
		model.addAttribute("entityListssi", entityListssi);
		...
		if (!entityList.isEmpty()) {
			...
			model.addAttribute("ACCOUNTCODEList", reconciliationService.listBySql("select distinct ACCOUNT_CODE from CUX_SSI_BAL where ENTITY_CODE in ("+entitys+")"));
			model.addAttribute("ACCOUNTDESCList", reconciliationService.listBySql("select distinct ACCOUNT_NAME from CUX_SSI_BAL where ENTITY_CODE in ("+entitys+")"));
		}
		return "/ebs/ssireconciliation/index";
	}
	...
	public String list(Model model,HttpServletRequest request,PageRequest pageRequest,String entity,String date,String ACCOUNTCODE,String ACCOUNTDESC,String ISICP,String CUSTNAME,String TC) {
		try {
			...
			Page<SSIReconciliation> page = reconciliationService.findPageByHQL(pageRequest, filters);
			model.addAttribute("page", page);
		} catch (Exception e) {
			logger.error("查询科目余额表列表失败:", e);
		}
		return "/ebs/ssireconciliation/list";
	}
	...
	public String upload(HttpServletRequest request,AjaxResult result,@Log(name = "年月") String date,@Log(name = "公司编码") String[] entitys) {
		...
		try {
			...
			if (mutipartFiles != null && mutipartFiles.size() > 0) {
				...
				List<SSIReconciliation> list=new ArrayList<SSIReconciliation>();
				...
				for (int i = 1; i < rowNum; i++) {
					...
					SSIReconciliation detail=new SSIReconciliation();
					detail.setYEAR(year);
	...
	public synchronized String download(HttpServletRequest request,HttpServletResponse response,PageRequest pageRequest,AjaxResult result,
		...
		try {
			...
			List<SSIReconciliation> list = reconciliationService.findPageByHQL(pageRequest, filters).getResult();
			
			if (CollectionUtils.isNotEmpty(list)) {
				String realPath=request.getRealPath("");
				File file=new File(request.getRealPath("")+File.separator+"static"+File.separator+"template"+File.separator+"ebs"+File.separator+EXCEL_NAME+"_downloadSSI.xlsx");
				...
				int rowIndex=1;
				for (SSIReconciliation reconciliation : list) {
