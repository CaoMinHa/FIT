...
public class PoEmailService extends BaseService<PoEmailLog> {
    ...
    public AjaxResult sendEmail(AjaxResult ajaxResult, String emailGroup, String title,String year, String content,String endDate){
        ...
        if(emailListC.size()==0){
            ...
            content=content.replace("\n","</br>");
            content=content.replace(" ","&nbsp;");
            Boolean isSend = EmailUtil.emailsMany(emailListC, title,content+"</br>&nbsp;&nbsp;<a href=\""+accessUrl+"\" style=\"color: blue;\">接口平臺</a><br></br>接口平臺登錄賬號是EIP賬號，密碼默認11111111，登錄如有問題，請聯系顧問，郵箱：brian.pr.chen@fit-foxconn.com。<br></br>Best Regards!");
            if(isSend){
    ...
    public void sendEmailTiming(String username,String content,String title){
        if(username.length()>1){
            ...
            List<String> emailListC=poTableService.listBySql(sqlC);
            emailListC=emailListC.stream().distinct().collect(Collectors.toList());
            EmailUtil.emailsMany(emailListC,title,content+"</br>&nbsp;&nbsp;<a href=\""+accessUrl+"\" style=\"color: blue;\">接口平臺</a><br></br>接口平臺登錄賬號是EIP賬號，密碼默認11111111，登錄如有問題，請聯系顧問，郵箱：brian.pr.chen@fit-foxconn.com。<br></br>Best Regards!",list);
        }
    }
    //定时任务发送邮件提醒CC
    public void sendEmailTimingCC(String username,String ccUserName,String content,String title){
        ...
        EmailUtil.emailsManyCC(emailVal.substring(0,emailVal.length()-1),emailValCC.substring(0,emailValCC.length()-1),title,content+"</br>&nbsp;&nbsp;<a href=\""+accessUrl+"\" style=\"color: blue;\">接口平臺</a><br></br>接口平臺登錄賬號是EIP賬號，密碼默認11111111，登錄如有問題，請聯系顧問，郵箱：brian.pr.chen@fit-foxconn.com。<br></br>Best Regards!",list);
    }
}