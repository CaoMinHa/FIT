...
public class DataDisseminationEmailService {
    ...
    public Model index(Model model){
        ...
        List<String> list=poTableDao.listBySql(sql);
...
//                sqlEmail="select BI_USER,BI_USERNAME,EMAIL,BI_GROUP,BI_PORTALPATH from BIDEV.Bi_user_list u where u.EMAIL is not null and instr(';'||u.BI_GROUP||';',';menu_plUnit;') > 0 and instr(';'||u.BI_GROUP||';',';SY_SBU;') <= 0 and u.BI_USER in('Maggie','Amber')";
            ...
            List<Map> emailListC=poTableDao.listMapBySql(sqlEmail);
            if(null!=emailListC&&emailListC.size()>0){
                ...
                for (Map map:emailListC) {
                    ...
                    List<String> rtUserEmailAuthorities= poTableDao.listBySql(sqlEmail);
                    ...
                    if(null==rtUserEmailAuthorities||rtUserEmailAuthorities.size()==0){
                            ...
                            content+=date+"FIT"+typeVal+"已發佈，請點擊以下鏈接登錄BI平臺進行查看，謝謝。"+message+"<br></br>&nbsp;&nbsp;&nbsp;<b>Link to:</b>&nbsp;<a href=\"https://bi.one-fit.com/analytics\" style=\"color: blue;\">FIT"+typeVal+"</a><br></br>BI平臺登錄賬號及密碼是EIP賬號及密碼，登錄如有問題，請聯系邮箱，郵箱：natalie.sy.tseng@fit-foxconn.com<br></br><br>Best Regards!";
                    }else{
                            title=date+rtUserEmailAuthorities.toString()+typeVal;
                            content+=date+rtUserEmailAuthorities.toString()+typeVal+"已發佈，請點擊以下鏈接登錄BI平臺進行查看，謝謝。"+message+"<br></br>&nbsp;&nbsp;&nbsp;<b>Link to:</b>&nbsp;<a href=\"https://bi.one-fit.com/analytics\" style=\"color: blue;\">"+rtUserEmailAuthorities.toString()+typeVal+"</a><br></br>BI平臺登錄賬號及密碼是EIP賬號及密碼，登錄如有問題，請聯系邮箱，郵箱：natalie.sy.tseng@fit-foxconn.com<br></br><br>Best Regards!";
                    }
                    System.out.print("发送邮件："+map.get("EMAIL").toString()+"****主题："+title+"****内容："+content);
                        if(type.equals("menu_plUnit")){
                            this.updateSql("update epmexp.cux_pl_default_bi set send_email_flag='N' where send_email_flag='Y' and YEARS||'-'||period='"+yearMonth+"'");
                            this.updateSql("update BIDEV.Bi_user_list set BI_PORTALPATH='/shared/FIT-BI Platform v2/01.分析/PL/D.損益單位損益表' where instr(';'||BI_GROUP||';',';menu_plUnit;') > 0  ");
                        }else if(type.equals("SY_SBU")){
                            this.updateSql("update epmexp.cux_pl_default_bi set send_email_flag='N' where send_email_flag='Y' and YEARS||'-'||period='"+yearMonth+"'");
                            this.updateSql("update BIDEV.Bi_user_list  set BI_PORTALPATH='/shared/FIT-BI Platform v2/01.分析/PL/D.SBU损益表' where instr(';'||BI_GROUP||';',';SY_SBU;') > 0  ");
                        }
    ...
    /**損益表获取郵件正文數值内容*/
    private String message(String yearMonth,String userCode,String type) throws Exception {
        String message="";
...
//            String sqlEmail="select BI_USER,BI_USERNAME,EMAIL,BI_GROUP,BI_PORTALPATH from BIDEV.Bi_user_list u where u.EMAIL is not null and u.BI_USER in('Maggie','Amber')";
            ...
            if(null!=emailListC&&emailListC.size()>0) {
                ...
                String content =yearMonth+"损益Rawdata数据已發佈，請點擊以下鏈接登錄BI平臺進行檢查，謝謝。<br></br>&nbsp;&nbsp;&nbsp;<b>Link to:</b>&nbsp;<a href=\"https://bi.one-fit.com/analytics\" style=\"color: blue;\">损益Rawdata</a><br></br>BI平臺登錄賬號及密碼是EIP賬號及密碼，系統登錄、數據核對如有問題，請聯系邮箱 , 分機 5070-32202 , 郵箱：natalie.sy.tseng@fit-foxconn.com。<br></br><br>Best Regards!";
    ...
    private AjaxResult biBuSendEamil(String yearMonth,String type,AjaxResult ajaxResult) {
        try {
            String [] period=yearMonth.split("-");
            String date=period[0]+"年";
            String month=period[1];
...
//                String sqlEmail="select BI_USER,BI_USERNAME,EMAIL,BI_GROUP,BI_PORTALPATH from BIDEV.Bi_user_list u where u.EMAIL is not null and u.BI_USER in('Maggie','Amber') and (instr(';'||u.BI_GROUP||';',';PL_BU;') > 0 or instr(';'||u.BI_GROUP||';',';PL_BU1;') > 0 or instr(';'||u.BI_GROUP||';',';PL_BU2;') > 0)";
                ...
                if (null != emailListC && emailListC.size() > 0) {
                    ...
                    String content = "";
                    if ("BS".equals(type)) {
                        this.updateSql("update epmexp.cux_bs_default_bi set send_email_flag='N' where send_email_flag='Y' and YEARS||'-'||period='"+yearMonth+"'");
                        this.updateSql("update  BIDEV.Bi_user_list  set BI_PORTALPATH='/shared/FIT-BI Platform v2/01.分析/BS/D.BU資產負債表' where instr(';'||BI_GROUP||';',';PL_BU;') > 0 ");
                        ...
                        title = date + "BU資產負債表";
                        content += date + "BU資產負債表已發佈，請點擊以下鏈接登錄BI平臺進行查看，謝謝。<br></br>&nbsp;&nbsp;&nbsp;<b>Link to:</b>&nbsp;<a href=\"https://bi.one-fit.com/analytics\" style=\"color: blue;\">BU資產負債表</a><br></br>BI平臺登錄賬號及密碼是EIP賬號及密碼，登錄如有問題，請聯系邮箱，郵箱：natalie.sy.tseng@fit-foxconn.com<br></br><br>Best Regards!";
                        ...
                        title = date + "BU现金流量表";
                        content += date + "BU现金流量表已發佈，請點擊以下鏈接登錄BI平臺進行查看，謝謝。<br></br>&nbsp;&nbsp;&nbsp;<b>Link to:</b>&nbsp;<a href=\"https://bi.one-fit.com/analytics\" style=\"color: blue;\">BU现金流量表</a><br></br>BI平臺登錄賬號及密碼是EIP賬號及密碼，登錄如有問題，請聯系邮箱，郵箱：natalie.sy.tseng@fit-foxconn.com<br></br><br>Best Regards!";
                    }
